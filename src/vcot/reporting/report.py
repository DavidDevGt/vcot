...
def build_report(records: List[dict], runs: Optional[List[dict]] = None) -> dict:
...
def render_markdown(report: dict) -> str:
    ...
    # 6. Ledger
    L.append("## 6. Registro de ejecuciones")
    L.append("")
    ledger = report.get("ledger")
    if ledger:
        L.append(
            f"{ledger['n_runs']} ejecuciones registradas  ·  "
            f"coste acumulado en ledger: ${ledger['total_cost_usd']:.4f}  ·  "
            f"por tipo: {ledger['by_kind']}"
        )
        L.append("")
        L.append("| Fecha | Tipo | Modelo | GPU | Ítems | Coste | Dur | Estado |")
        L.append("|---|---|---|---|--:|--:|--:|---|")
        for r in ledger["runs"][-20:]:
            L.append(
                f"| {r.get('started_at','')} | {r.get('kind','')} | {r.get('model') or '—'} | "
                f"{r.get('gpu') or '—'} | {r.get('n_items',0)} | "
                f"${float(r.get('total_cost_usd',0)):.4f} | {r.get('duration_s',0):.1f}s | "
                f"{r.get('status','')} |"
            )
    else:
...
def write_bundle(
    ...
    if runs:
        ...
        with open(runs_csv, "w", encoding="utf-8", newline="") as fh:
            w = csv.writer(fh)
            w.writerow(["started_at", "kind", "model", "gpu", "n_items", "total_cost_usd", "duration_s", "status"])
            for r in runs:
                w.writerow([
                    r.get("started_at", ""), r.get("kind", ""), r.get("model", ""),
                    r.get("gpu", ""), r.get("n_items", 0), r.get("total_cost_usd", 0.0),
                    r.get("duration_s", 0.0), r.get("status", ""),
                ])
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    report = build_report(records, runs)
